...
def init_registration(message: Message, bot: telebot.TeleBot):
...
def block_interface(message: Message, bot: telebot.TeleBot):
    Red.set(f"lock-interface-{message.from_user.id}", "true")
    return bot.send_message(chat_id=message.chat.id, text=locale.read('pay_proof_message'))
